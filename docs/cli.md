...
| 19 | `inbox`      | `apps/cli/src/commands/inbox.ts`                | Drain/clean the local inbox (`.cognit/inbox/`): `--process`, `--watch`, `--reprocess`, `--status`, `--install-watch`, `--clean-tmp` (+ `--dry-run`, `--max-age-days`). Read commands drain lazily via `inbox.auto_drain` (default on). See `docs/hooks/README.md`. |
...
| 30 | `dashboard`  | `apps/cli/src/commands/dashboard.ts`            | Run the dashboard SPA on `127.0.0.1:6970` and API for the current Cognit root on `127.0.0.1:6971` (see `docs/dashboard.md`). |
